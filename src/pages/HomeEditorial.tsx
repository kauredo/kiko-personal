...
import { ArrowDown, Instagram, Youtube, Music2, Linkedin, Mail } from "lucide-react";
...
export function HomeEditorial() {
  ...
  return (
    <div className="min-h-screen" style={{ background: bg, color: fg, fontFamily: bodyFont }}>
      ...
      {/* ═══ ABOUT ═══ */}
      <section id="editorial-about" className="px-6 md:px-16 lg:px-24 xl:px-32 py-24 md:py-32">
      ...
      {/* ═══ SERVICES ═══ */}
      <section id="editorial-services" className="px-6 md:px-16 lg:px-24 xl:px-32 py-16 md:py-24" style={{ borderTop: `1px solid ${border}` }}>
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="columns-2 gap-4 md:columns-3">
              {photos.map((item, i) => (
                ...
                >
                  {item.imageUrl ? (
                    <img src={item.imageUrl} alt={item.title} loading="lazy" className="w-full object-cover transition-transform duration-700 group-hover:scale-105" />
                  ) : (
      ...
      {/* ═══ VIDEOS ═══ */}
      {videos.length > 0 && (
        <section className="px-6 md:px-12 lg:px-24 xl:px-32 py-16 md:py-24" style={{ borderTop: `1px solid ${border}` }}>
          <div className="mx-auto max-w-6xl">
            <p className="mb-3 text-xs uppercase" style={{ color: primary, letterSpacing: "0.2em" }}>Videos</p>
            <h2 className="mb-16" style={{ fontFamily: headingFont, fontSize: "clamp(2rem, 5vw, 3.5rem)", lineHeight: 1.1 }}>In motion</h2>
            <div className="grid gap-4 md:grid-cols-2">
              {videos.map((item) => (
                item.imageUrl ? (
                  <VideoCard key={item.id} src={item.imageUrl!} title={item.title} onClick={() => openGallery(allMedia, allMedia.findIndex(m => m.id === item.id))} style={{ background: card, border: `1px solid ${border}` }} />
                ) : (
                  <div key={item.id} style={{ aspectRatio: "16/9", background: card, border: `1px solid ${border}` }}>
      ...
      {music.length > 0 && (
        <section className="px-6 md:px-12 lg:px-24 xl:px-32 py-16 md:py-24" style={{ borderTop: `1px solid ${border}` }}>
          <div className="mx-auto max-w-4xl">
            <p className="mb-3 text-xs uppercase" style={{ color: primary, letterSpacing: "0.2em" }}>Music</p>
            <h2 className="mb-12" style={{ fontFamily: headingFont, fontSize: "clamp(2rem, 5vw, 3.5rem)", lineHeight: 1.1 }}>Listen</h2>
            {music.map((item) => (
      ...
      {experiences.length > 0 && (
        <section id="editorial-work" className="px-6 md:px-16 lg:px-24 xl:px-32 py-16 md:py-24" style={{ borderTop: `1px solid ${border}` }}>
          <div className="mx-auto max-w-5xl">
            <p className="mb-3 text-xs uppercase" style={{ color: primary, letterSpacing: "0.2em" }}>Experience</p>
            <h2 className="mb-12" style={{ fontFamily: headingFont, fontSize: "clamp(2rem, 5vw, 3.5rem)", lineHeight: 1.1 }}>
              The work so far
            </h2>
            {experiences.map((w, i) => (
              <div key={i} className="flex flex-col gap-1 py-6 md:flex-row md:items-baseline md:justify-between" style={{ borderTop: `1px solid ${border}` }}>
                <div>
      ...
      {testimonials.length > 0 && (
        <section className="px-6 md:px-16 lg:px-24 xl:px-32 py-20 md:py-28" style={{ background: card }}>
      ...
      {/* ═══ EVENTS ═══ */}
      {events.length > 0 && (
        <section id="editorial-events" className="px-6 md:px-16 lg:px-24 xl:px-32 py-16 md:py-24">
          <div className="mx-auto max-w-5xl">
            <div className="mb-12 flex items-end justify-between">
              <div>
                <p className="mb-3 text-xs uppercase" style={{ color: primary, letterSpacing: "0.2em" }}>Live</p>
                <h2 style={{ fontFamily: headingFont, fontSize: "clamp(2rem, 5vw, 3.5rem)", lineHeight: 1.1 }}>Events</h2>
              </div>
            </div>
            {events.map((e, i) => (
              <div key={i} className="flex flex-col gap-2 py-6 md:flex-row md:items-center md:justify-between" style={{ borderTop: `1px solid ${border}` }}>
      ...
      <section id="editorial-contact" className="px-6 md:px-12 lg:px-24 xl:px-32 py-24 text-center md:py-32" style={{ borderTop: `1px solid ${border}` }}>
        ...
        <a
          ...
          className="inline-flex items-center gap-2 border px-10 py-4 text-xs uppercase transition-colors hover:bg-[oklch(0.72_0.12_70)] hover:text-[oklch(0.10_0.01_55)]"
        ...
        {settings.socialLinks.length > 0 && (
          <div className="flex items-center gap-4">
            {settings.socialLinks.map((link) => {
              ...
              return (
                <a key={link.platform} href={link.url} target="_blank" rel="noopener noreferrer" style={{ color: mutedFg }} className="flex h-10 w-10 items-center justify-center transition-opacity hover:opacity-70">
                  <Icon size={18} />
                </a>
              );
            })}
